...
import datetime
import csv
import random # Added for ML fallback
import math # Added for pagination
import requests # Added for REST Firebase Auth
...
def customer_register():
    try:
        ...
        existing_email = list(db.collection('customers').where('email', '==', email).stream())
        if existing_email:
            return jsonify({"success": False, "error": "Email already registered"}), 400

        # Check for name uniqueness (Case Insensitive)
        name_lower = name.lower()
        existing_name = list(db.collection('customers').where('name_lower', '==', name_lower).stream())
        if existing_name:
...
def customer_login():
    try:
        ...
        users = list(db.collection('customers')
                     .where('email', '==', email)
...
def admin_login():
    try:
        ...
        # Hardcoded backend bypass for emergency access
        if email == 'admin@vsp.com' and password == 'admin123':
            return jsonify({
                "success": True,
                "user": {
                    "id": "admin-1",
        ...
        API_KEY = "AIzaSyBouC7mbVqUzBAYQ8AAnT4bwE3F9RrV4NQ"
        ...
        if current_time - forecast_cache["last_updated"] > 3600 and not forecast_cache["is_calculating"]:
            ...
            def run_ml_background():
                try:
                    ...
                    csv_path = "/Users/yugendharanmohan/Downloads/project 2/vellore-spun-pipes-project/backend/synthetic_pipe_sales_march_2026.csv"
                    try:
                        NAME_MAPPING = {
